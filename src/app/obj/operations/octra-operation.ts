...
export class OCTRAOperation extends ToolOperation {
  ...
  public constructor(name: string, title?: string, shortTitle?: string, task?: Task, state?: TaskState, id?: number) {
    ...
    this._description = 'A literal transcript contains the verbatim text of a speech recording. These transcripts are either generated by Automatic Speech Recognition or they are created manually from scratch. The editor Octra allows you to correct or create such transcripts.';
  }
  ...
  public getToolURL(): string {
    if (!((<UploadOperation> this.operations[0]).wavFile === null || (<UploadOperation> this.operations[0]).wavFile === undefined)) {
      ...
      const host = `host=${encodeURIComponent(AppInfo.getLanguageByCode(this.task.language).host)}`;
  ...
  public fromAny(operationObj: any, task: Task): OCTRAOperation {
    const result = new OCTRAOperation(operationObj.name, this.title, this.shortTitle, task, operationObj.state, operationObj.id);
  ...
  public clone(task?: Task): OCTRAOperation {
    ...
    return new OCTRAOperation(this.name, this.title, this.shortTitle, selected_task, this.state);
  }
}
