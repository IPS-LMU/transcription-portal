<?php
// SETTINGS
// $to_email = 'octra@phonetik.uni-muenchen.de';
$to_email = 'octra@phonetik.uni-muenchen.de';
...
function getHTMLText($data, $screenshots)
{
  ...
  <div class="container">
    ...
    <span class="url">Installation: <a
        href="<?php echo $data["report"]["ohportal"]["url"]; ?>"><?php echo $data["report"]["ohportal"]["url"]; ?></a></span>
    ...
    <?php if (!empty($data["report"])):
      ...
      <table id="octra-table">
        <tbody>
        <?php foreach ($data["report"]["oh-portal"] as $key => $value): ?>
          <tr>
            <td><?php echo $key; ?></td>
...
if (!empty($data) && !is_null($headers["Authorization"]) && $headers["Authorization"] === $auth_token) {
  ...
  if (!is_null($data["additional_information"]) && !is_null($data["description"])) {
    ...
    $topic = "OH-Portal Feedback [$timestamp]";


    if (empty($from_email)) {
      $from_email = "do-not-reply@ohportal.net";
    }
