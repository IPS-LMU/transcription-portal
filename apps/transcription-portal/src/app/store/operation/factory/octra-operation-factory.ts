...
import { transactions } from 'jodit/types/core/selection/style/transactions';

export type OctraOperation = StoreTaskOperation<any, OctraOperation>;

export class OctraOperationFactory extends ToolOperationFactory<OctraOperation> {
  ...
  public async getToolURL(audioFile: StoreAudioFile, transcriptFile: StoreFile | undefined, httpClient: HttpClient): Promise<string> {
    if (audioFile?.online && audioFile?.url) {
      ...
      if (transcriptFile) {
        transcriptContent = transcriptFile.content;
        if (transcriptFile.url) {
          try {
            transcriptContent = await downloadFile<string>(transcriptFile.url, 'text');
          } catch (e) {
            console.error(e);
          }
        }
